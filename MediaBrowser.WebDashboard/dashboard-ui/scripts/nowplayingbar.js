define(["datetime", "userdataButtons", "itemHelper", "events", "browser", "imageLoader", "playbackManager", "nowPlayingHelper", "apphost", "dom", "connectionManager", "paper-icon-button-light"], function (datetime, userdataButtons, itemHelper, events, browser, imageLoader, playbackManager, nowPlayingHelper, appHost, dom, connectionManager) {
    ...
    function slideDown(elem) {
        void elem.offsetWidth, requestAnimationFrame(function () {
            elem.classList.add("nowPlayingBar-hidden"), dom.addEventListener(elem, dom.whichTransitionEvent(), onSlideDownComplete, {
                once: !0
            })
        })
    }
    ...
    function slideUp(elem) {
        setTimeout(function () {
            dom.removeEventListener(elem, dom.whichTransitionEvent(), onSlideDownComplete, {
                once: !0
            }), elem.classList.remove("hide"), void elem.offsetWidth, requestAnimationFrame(function () {
    ...
    function bindEvents(elem) {
        ...
        elem.querySelector(".nextTrackButton").addEventListener("click", function () {
            ...
            showRemoteControl()
        }), toggleRepeatButton = elem.querySelector(".toggleRepeatButton"), toggleRepeatButton.addEventListener("click", function () {
    ...
    function showRemoteControl(tabIndex) {
        tabIndex ? Dashboard.navigate("nowplaying.html?tab=" + tabIndex) : Dashboard.navigate("nowplaying.html")
    }
    ...
    function getNowPlayingBar() {
        return nowPlayingBarElement ? Promise.resolve(nowPlayingBarElement) : new Promise(function (resolve, reject) {
            require(["appfooter-shared", "itemShortcuts", "css!css/nowplayingbar.css", "emby-slider"], function (appfooter, itemShortcuts) {
                ...
                return (nowPlayingBarElement = parentContainer.querySelector(".nowPlayingBar")) ? void resolve(nowPlayingBarElement) : (parentContainer.insertAdjacentHTML("afterbegin", getNowPlayingBarHtml()), nowPlayingBarElement = parentContainer.querySelector(".nowPlayingBar"), browser.safari && browser.slow && nowPlayingBarElement.classList.add("noMediaProgress"), itemShortcuts.on(nowPlayingBarElement), bindEvents(nowPlayingBarElement), void resolve(nowPlayingBarElement))
            })
        })
    }
    ...
    function updatePlayerStateInternal(event, state) {
        ...
        currentPlayerSupportedCommands = supportedCommands, supportedCommands.indexOf("SetRepeatMode") == -1 ? toggleRepeatButton.classList.add("hide") : toggleRepeatButton.classList.remove("hide"), updateRepeatModeDisplay(playState.RepeatMode), updatePlayerVolumeState(playState.IsMuted, playState.VolumeLevel), positionSlider && !positionSlider.dragging && (positionSlider.disabled = !playState.CanSeek);
        var nowPlayingItem = state.NowPlayingItem || {};
        updateTimeDisplay(playState.PositionTicks, nowPlayingItem.RunTimeTicks), updateNowPlayingInfo(state)
    }

    function updateRepeatModeDisplay(repeatMode) {
        "RepeatAll" == repeatMode ? (toggleRepeatButtonIcon.innerHTML = "repeat", toggleRepeatButton.classList.add("repeatActive")) : "RepeatOne" == repeatMode ? (toggleRepeatButtonIcon.innerHTML = "repeat_one", toggleRepeatButton.classList.add("repeatActive")) : (toggleRepeatButtonIcon.innerHTML = "repeat", toggleRepeatButton.classList.remove("repeatActive"))
    }
    ...
    function updateNowPlayingInfo(state) {
        ...
        nowPlayingTextElement.innerHTML = nowPlayingItem ? nowPlayingHelper.getNowPlayingNames(nowPlayingItem).map(function (nowPlayingName) {
            return nowPlayingName.item ? "<div>" + getTextActionButton(nowPlayingName.item, nowPlayingName.text) + "</div>" : "<div>" + nowPlayingName.text + "</div>"
        }).join("") : "";
        var imgHeight = 70,
            url = nowPlayingItem ? seriesImageUrl(nowPlayingItem, {
                height: imgHeight
            }) || imageUrl(nowPlayingItem, {
                height: imgHeight
            }) : null;
        url !== currentImgUrl && (currentImgUrl = url, imageLoader.lazyImage(nowPlayingImageElement, url)), nowPlayingItem.Id ? ApiClient.getItem(Dashboard.getCurrentUserId(), nowPlayingItem.Id).then(function (item) {
    ...
    function hideNowPlayingBar() {
        ...
        elem && (document.body.classList.contains("hiddenNowPlayingBar") ? (dom.removeEventListener(elem, dom.whichTransitionEvent(), onSlideDownComplete, {
            once: !0
        }), elem.classList.add("hide"), elem.classList.add("nowPlayingBar-hidden")) : slideDown(elem))
    }

    function onPlaybackStopped(e, state) {
        console.log("nowplaying event: " + e.type);
        var player = this;
        player.isLocalPlayer ? "Audio" !== state.NextMediaType && hideNowPlayingBar() : state.NextMediaType || hideNowPlayingBar()
    }
    ...
    function onStateChanged(event, state) {
        var player = this;
        return state.NowPlayingItem ? player.isLocalPlayer && state.NowPlayingItem && "Video" == state.NowPlayingItem.MediaType ? void hideNowPlayingBar() : (isEnabled = !0, nowPlayingBarElement ? void updatePlayerStateInternal(event, state) : void getNowPlayingBar().then(function () {
    ...
    function bindToPlayer(player) {
        ...
        }), events.on(player, "playbackstart", onPlaybackStart), events.on(player, "statechange", onPlaybackStart), events.on(player, "repeatmodechange", onRepeatModeChange), events.on(player, "playbackstop", onPlaybackStopped), events.on(player, "volumechange", onVolumeChanged), events.on(player, "pause", onPlayPauseStateChanged), events.on(player, "playing", onPlayPauseStateChanged), events.on(player, "timeupdate", onTimeUpdate)))
    }
