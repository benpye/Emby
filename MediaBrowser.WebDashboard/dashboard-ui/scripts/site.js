...
var Dashboard = {
    isConnectMode: function () {
        if (AppInfo.isNativeApp) return true;
    ...
    getToolsMenuHtml: function (page) {
        ...
        for (menuHtml += '<div class="drawerContent">', i = 0, length = items.length; i < length; i++) {
            ...
            if (item.href) {
                ...
                menuHtml += item.name, menuHtml += "</div>";
            }
        }
...
(function () {
    ...
    function initRequire() {
        ...
        var paths = {
                ...
                hlsjs: bowerPath + "/hlsjs/dist/hls.min",
                wakeonlan: apiClientBowerPath + "/wakeonlan"
            };
    ...
    function updateAppSettings(appSettings) {
        appSettings.enableExternalPlayers = function (val) {
            ...
            return "true" === appSettings.get("externalplayers");
        };
        return appSettings;
    }
...
pageClassOn("viewinit", "page", function () {
    ...
    if (!current) {
        var newTheme;
        newTheme = page.classList.contains("libraryPage") ? "b" : "a";
