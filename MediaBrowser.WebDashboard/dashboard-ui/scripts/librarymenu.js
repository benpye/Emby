define(["layoutManager", "viewManager", "libraryBrowser", "embyRouter", "playbackManager", "browser", "paper-icon-button-light", "material-icons", "scrollStyles"], function (layoutManager, viewManager, libraryBrowser, embyRouter, playbackManager, browser) {
    ...
    function updateUserInHeader(user) {
        ...
        if (header) {
            var hasImage, headerUserButton = header.querySelector(".headerUserButton");
    ...
    function updateLocalUser(user) {
        var header = document.querySelector(".viewMenuBar"),
            headerSearchButton = header.querySelector(".headerSearchButton"),
            btnCast = header.querySelector(".btnCast"),
            dashboardEntryHeaderButton = header.querySelector(".dashboardEntryHeaderButton");
        user ? (btnCast.classList.remove("hide"), headerSearchButton && headerSearchButton.classList.remove("hide"), dashboardEntryHeaderButton && (user.Policy.IsAdministrator ? dashboardEntryHeaderButton.classList.remove("hide") : dashboardEntryHeaderButton.classList.add("hide")), require(["apphost"], function (apphost) {
            apphost.supports("voiceinput") ? header.querySelector(".headerVoiceButton").classList.add("hide") : header.querySelector(".headerVoiceButton").classList.add("hide")
        })) : (btnCast.classList.add("hide"), header.querySelector(".headerVoiceButton").classList.add("hide"), headerSearchButton && headerSearchButton.classList.add("hide"), dashboardEntryHeaderButton && dashboardEntryHeaderButton.classList.add("hide"))
    }
    ...
    function bindMenuEvents() {
        ...
        headerVoiceButton && headerVoiceButton.addEventListener("click", showVoice);
        ...
        headerAppsButton && headerAppsButton.addEventListener("click", onHeaderAppsButtonClick);
        var viewMenuBar = document.querySelector(".viewMenuBar");
        initHeadRoom(viewMenuBar), viewMenuBar.querySelector(".btnNotifications").addEventListener("click", function () {
    ...
    function onMainDrawerOpened() {
        browser.mobile && document.body.classList.add("bodyWithPopupOpen")
    }
        ...
        return apiClient.getUserViews({}, userId).then(function (result) {
            for (var items = result.Items, list = [], i = 0, length = items.length; i < length; i++) {
                ...
                if (list.push(view), "livetv" == view.CollectionType) {
                    view.ImageTags = {}, view.icon = "live_tv", view.onclick = "LibraryBrowser.showTab('livetv.html', 0);";
                    var guideView = Object.assign({}, view);
                    guideView.Name = Globalize.translate("ButtonGuide"), guideView.ImageTags = {}, guideView.icon = "dvr", guideView.url = "livetv.html?tab=1", guideView.onclick = "LibraryBrowser.showTab('livetv.html', 1);", list.push(guideView)
                }
            }
            return list
        })
    ...
    function updateLibraryMenu(user) {
        ...
        user.Policy.EnableSync ? showBySelector(".lnkSyncToOtherDevices", !0) : showBySelector(".lnkSyncToOtherDevices", !1), require(["apphost"], function (appHost) {
            user.Policy.EnableSync && appHost.supports("sync") ? showBySelector(".lnkManageOffline", !0) : showBySelector(".lnkManageOffline", !1)
        });
    ...
    function updateCastIcon() {
        ...
        if (btnCast) {
            ...
            info && !info.isLocalPlayer ? (btnCast.querySelector("i").icon = "cast_connected", btnCast.classList.add("btnActiveCast"), context.querySelector(".headerSelectedPlayer").innerHTML = info.deviceName || info.name) : (btnCast.querySelector("i").innerHTML = "cast", btnCast.classList.remove("btnActiveCast"), context.querySelector(".headerSelectedPlayer").innerHTML = "")
        }
    }
    ...
    function setDrawerClass(page) {
        var admin = !1;
        page || (page = viewManager.currentView()), page && page.classList.contains("type-interior") && (admin = !0), loadNavDrawer().then(function () {
            admin ? (navDrawerElement.classList.add("adminDrawer"), navDrawerElement.classList.remove("darkDrawer")) : (navDrawerElement.classList.add("darkDrawer"), navDrawerElement.classList.remove("adminDrawer"))
        })
    }
    ...
    function getNavDrawerOptions() {
        ...
        drawerWidth = Math.max(drawerWidth, 240), drawerWidth = Math.min(drawerWidth, 320);
        var disableEdgeSwipe = !1;
        return browser.safari && (disableEdgeSwipe = !0), {
    ...
    function loadNavDrawer() {
        return navDrawerInstance ? Promise.resolve(navDrawerInstance) : new Promise(function (resolve, reject) {
            navDrawerElement = document.querySelector(".mainDrawer"), navDrawerScrollContainer = navDrawerElement.querySelector(".scrollContainer"), require(["navdrawer"], function (navdrawer) {
                navDrawerInstance = new navdrawer(getNavDrawerOptions()), navDrawerElement.classList.remove("hide"), resolve(navDrawerInstance)
            })
        })
    }
    var navDrawerElement, navDrawerScrollContainer, navDrawerInstance, mainDrawerButton, enableBottomTabs = browser.mobile,
        enableLibraryNavDrawer = !enableBottomTabs,
    ...
    return window.LibraryMenu = {
        ...
        setTabs: function (type, selectedIndex, builder) {
            var viewMenuBarTabs;
            return type ? (viewMenuBarTabs = document.querySelector(".viewMenuBarTabs"), LibraryMenu.tabType || viewMenuBarTabs.classList.remove("hide"), void require(["emby-tabs", "emby-button"], function () {
                if (LibraryMenu.tabType != type) {
                    var index = 0;
                    return viewMenuBarTabs.innerHTML = '<div is="emby-tabs"><div class="emby-tabs-slider">' + builder().map(function (t) {
                        var tabClass = selectedIndex == index ? "emby-tab-button emby-tab-button-active" : "emby-tab-button",
                            tabHtml = '<button onclick="Dashboard.navigate(this.getAttribute(\'data-href\'));" data-href="' + t.href + '" is="emby-button" class="' + tabClass + '" data-index="' + index + '"><div class="emby-button-foreground">' + t.name + "</div></button>";
                        return index++, tabHtml
                    }).join("") + "</div></div>", document.body.classList.add("withTallToolbar"), void(LibraryMenu.tabType = type)
                }
                viewMenuBarTabs.querySelector('[is="emby-tabs"]').selectedIndex(selectedIndex), LibraryMenu.tabType = type
            })) : void(LibraryMenu.tabType && (document.body.classList.remove("withTallToolbar"), viewMenuBarTabs = document.querySelector(".viewMenuBarTabs"), viewMenuBarTabs.innerHTML = "", viewMenuBarTabs.classList.add("hide"), LibraryMenu.tabType = null))
        },
        ...
        setDrawerClass(), ConnectionManager.user(ConnectionManager.getApiClient(user.ServerId)).then(function (user) {
            refreshLibraryDrawer(user), updateUserInHeader(user)
        })
    }), Events.on(ConnectionManager, "localusersignedout", updateUserInHeader), Events.on(playbackManager, "playerchange", updateCastIcon), setDrawerClass(), enableBottomTabs && require(["appfooter-shared", "dockedtabs"], function (footer, dockedtabs) {
        new dockedtabs({
            appFooter: footer
        })
    }), LibraryMenu
