define(["appSettings", "paper-icon-button-light"], function (appSettings) {
    ...
    function updatePageStyle(page) {
        "1" == getParameterByName("showuser") ? (page.classList.add("libraryPage"), page.classList.add("noSecondaryNavPage"), page.classList.remove("standalonePage")) : (page.classList.add("standalonePage"), page.classList.remove("noSecondaryNavPage"), page.classList.remove("libraryPage"))
    }
    ...
    function getServerHtml(server) {
        var html = "";
        return html += '<div class="listItem serverItem" data-id="' + server.Id + '">', html += '<button type="button" is="emby-button" class="fab mini autoSize blue lnkServer" item-icon><i class="md-icon">wifi</i></button>', html += '<div class="listItemBody lnkServer">', html += '<a class="clearLink" href="#">', html += "<div>", html += server.Name, html += "</div>", html += "</a>", html += "</div>", server.Id && (html += '<button is="paper-icon-button-light" class="btnServerMenu autoSize"><i class="md-icon">more_vert</i></button>'), html += "</div>"
    }

    function renderServers(page, servers) {
        servers.length ? (page.querySelector(".noServersMessage").classList.add("hide"), page.querySelector(".serverList").classList.remove("hide")) : (page.querySelector(".serverList").classList.add("hide"), page.querySelector(".noServersMessage").classList.remove("hide"));
        var html = "";
        html += servers.map(getServerHtml).join(""), page.querySelector(".serverList").innerHTML = html
    }
    ...
    function showGeneralError() {
        Dashboard.hideLoadingMsg(), alertText(Globalize.translate("DefaultErrorMessage"))
    }
    ...
    return function (view, params) {
        function connectToServer(page, server) {
            Dashboard.showLoadingMsg(), ConnectionManager.connectToServer(server, {
        ...
        function acceptInvitation(page, id) {
            Dashboard.showLoadingMsg(), ConnectionManager.acceptServer(id).then(function () {
                Dashboard.hideLoadingMsg(), loadPage(page)
            }, function () {
                showGeneralError()
            })
        }
        ...
        function deleteServer(page, serverId) {
            Dashboard.showLoadingMsg(), ConnectionManager.deleteServer(serverId).then(function () {
                Dashboard.hideLoadingMsg(), loadPage(page)
            }, function () {
                showGeneralError()
            })
        ...
        function rejectInvitation(page, id) {
            Dashboard.showLoadingMsg(), ConnectionManager.rejectServer(id).then(function () {
                Dashboard.hideLoadingMsg(), loadPage(page)
            }, function () {
                showGeneralError()
            })
        ...
        function showServerMenu(elem) {
            var card = parentWithClass(elem, "serverItem"),
                page = parentWithClass(elem, "page"),
                serverId = card.getAttribute("data-id"),
                menuItems = [];
            menuItems.push({
        ...
        function getPendingInviteHtml(invite) {
            var html = "";
            return html += '<div class="listItem inviteItem" data-id="' + invite.Id + '">', html += '<button type="button" is="emby-button" class="fab mini autoSize blue lnkServer" item-icon><i class="md-icon">wifi</i></button>', html += '<div class="listItemBody">', html += "<div>", html += invite.Name, html += "</div>", html += "</div>", html += '<button is="paper-icon-button-light" class="btnInviteMenu autoSize"><i class="md-icon">more_vert</i></button>', html += "</div>"
        }

        function renderInvitations(page, list) {
            list.length ? page.querySelector(".invitationSection").classList.remove("hide") : page.querySelector(".invitationSection").classList.add("hide");
            var html = list.map(getPendingInviteHtml).join("");
            page.querySelector(".invitationList").innerHTML = html
        }

        function loadInvitations(page) {
            ConnectionManager.isLoggedIntoConnect() ? ConnectionManager.getUserInvitations().then(function (list) {
                renderInvitations(page, list)
            }) : renderInvitations(page, [])
        }

        function loadPage(page) {
            Dashboard.showLoadingMsg(), ConnectionManager.getAvailableServers().then(function (servers) {
                servers = servers.slice(0), cachedServers = servers, renderServers(page, servers), Dashboard.hideLoadingMsg()
            }), loadInvitations(page), ConnectionManager.isLoggedIntoConnect() ? page.querySelector(".connectLogin").classList.add("hide") : page.querySelector(".connectLogin").classList.remove("hide")
        }
        var cachedServers;
