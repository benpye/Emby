    ...
    return function (view, params) {
        ...
        function setPoster(item) {
            ...
            if (item) {
                ...
                if (imgUrl) return void(osdPoster.innerHTML = '<img src="' + imgUrl + '" />')
            }
            osdPoster.innerHTML = ""
        }
        ...
        function onPlaybackStopped(e, state) {
            currentRuntimeTicks = null, console.log("nowplaying event: " + e.type), "Video" !== state.NextMediaType && (view.removeEventListener("viewbeforehide", onViewHideStopPlayback), Emby.Page.back())
        }
        ...
        function bindToPlayer(player) {
            ...
            }), events.on(player, "playbackstart", onPlaybackStart), events.on(player, "playbackstop", onPlaybackStopped), events.on(player, "volumechange", onVolumeChanged), events.on(player, "pause", onPlayPauseStateChanged), events.on(player, "playing", onPlayPauseStateChanged), events.on(player, "timeupdate", onTimeUpdate), events.on(player, "fullscreenchange", updateFullscreenIcon)))
        }
        ...
        function updatePlayerStateInternal(event, state) {
            ...
            currentPlayerSupportedCommands = supportedCommands, updatePlayerVolumeState(playState.IsMuted, playState.VolumeLevel), nowPlayingPositionSlider && !nowPlayingPositionSlider.dragging && (nowPlayingPositionSlider.disabled = !playState.CanSeek), btnFastForward.disabled = !playState.CanSeek, btnRewind.disabled = !playState.CanSeek;
        ...
        var currentPlayer, isEnabled, currentItem, hideTimeout, lastMouseMoveData, currentPlayerSupportedCommands = [],
        ...
        view.addEventListener("viewbeforeshow", function (e) {
            ...
            browser.touch || playbackManager.playPause(currentPlayer), showOsd()
        ...
        }), view.querySelector(".btnAudio").addEventListener("click", showAudioTrackSelection), view.querySelector(".btnSubtitles").addEventListener("click", showSubtitleTrackSelection)
    }
});