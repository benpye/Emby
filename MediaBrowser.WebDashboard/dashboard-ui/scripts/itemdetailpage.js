define(["layoutManager", "cardBuilder", "datetime", "mediaInfo", "backdrop", "listView", "itemContextMenu", "itemHelper", "userdataButtons", "dom", "indicators", "apphost", "imageLoader", "libraryMenu", "globalize", "browser", "events", "scrollHelper", "playbackManager", "scrollStyles", "emby-itemscontainer", "emby-checkbox"], function (layoutManager, cardBuilder, datetime, mediaInfo, backdrop, listView, itemContextMenu, itemHelper, userdataButtons, dom, indicators, appHost, imageLoader, libraryMenu, globalize, browser, events, scrollHelper, playbackManager) {
    ...
    function reload(page, params) {
        Dashboard.showLoadingMsg(), getPromise(params).then(function (item) {
    ...
    function renderSyncLocalContainer(page, params, user, item) {
        return page.syncToggleInstance ? void page.syncToggleInstance.refresh(item) : void require(["syncToggle"], function (syncToggle) {
    ...
    function reloadFromItem(page, params, item) {
        ...
        LibraryBrowser.renderName(item, page.querySelector(".itemName"), !1, context), LibraryBrowser.renderParentName(item, page.querySelector(".parentName"), context), libraryMenu.setTitle(""), Dashboard.getCurrentUser().then(function (user) {
            ...
            user.Policy.IsAdministrator && groupedVersions.length ? page.querySelector(".splitVersionContainer").classList.remove("hide") : page.querySelector(".splitVersionContainer").classList.add("hide"), itemContextMenu.getCommands(getContextMenuOptions(item)).then(function (commands) {
    ...
    function renderLinks(linksElem, item) {
        var links = [];
        if (item.HomePageUrl && links.push('<a class="textlink" href="' + item.HomePageUrl + '" target="_blank">' + globalize.translate("ButtonWebsite") + "</a>"), item.ExternalUrls)
    ...
    function renderImage(page, item, user) {
        ...
        LibraryBrowser.renderDetailImage(container, item, user.Policy.IsAdministrator && "Photo" != item.MediaType, null, imageLoader, indicators)
    }
    ...
    function renderDetails(page, item, context, isStatic) {
        ...
        "Season" !== item.Type && "MusicAlbum" !== item.Type && "MusicArtist" !== item.Type || (overview.classList.add("detailsHiddenOnMobile"), externalLinksElem.classList.add("detailsHiddenOnMobile")), renderOverview([overview], item), renderAwardSummary(page.querySelector("#awardSummary"), item);
        ...
        renderStudios(page.querySelector(".itemStudios"), item, isStatic), renderUserDataIcons(page, item), renderLinks(externalLinksElem, item), page.querySelector(".criticRatingScore").innerHTML = (item.CriticRating || "0") + "%", item.CriticRatingSummary ? (page.querySelector("#criticRatingSummary").classList.remove("hide"), page.querySelector(".criticRatingSummaryText").innerHTML = item.CriticRatingSummary) : page.querySelector("#criticRatingSummary").classList.add("hide"), renderTags(page, item), renderSeriesAirTime(page, item, isStatic), renderDynamicMediaIcons(page, item) ? page.querySelector(".mediaInfoIcons").classList.remove("hide") : page.querySelector(".mediaInfoIcons").classList.add("hide");
    ...
    function renderMoreFromItems(page, item) {
        ...
        if (moreFromSection) return "MusicAlbum" == item.Type && item.AlbumArtists && item.AlbumArtists.length ? void ApiClient.getItems(Dashboard.getCurrentUserId(), {
            ...
            var html = "";
            html += enableScrollX() ? '<div is="emby-itemscontainer" class="hiddenScrollX itemsContainer">' : '<div is="emby-itemscontainer" class="itemsContainer vertical-wrap">';
        ...
        if (item.AirDays && item.AirDays.length && (html += 7 == item.AirDays.length ? "daily" : item.AirDays.map(function (a) {
            ...
            if (isStatic) html += " on " + item.Studios[0].Name;
                ...
                var context = inferContext(item),
                    href = LibraryBrowser.getHref(item.Studios[0], context);
    ...
    function renderChildren(page, item) {
        ...
        "Series" == item.Type ? promise = ApiClient.getSeasons(item.Id, {
            ...
            if ("MusicAlbum" == item.Type) html = listView.getListViewHtml({
                ...
                if ("Episode" === item.Type && childrenItemsContainer.classList.add("darkScroller"), scrollX = "Episode" == item.Type, browser.touch || (scrollClass = "smoothScrollX"), 1 == result.Items.length && "Episode" === item.Type) return;
                html = cardBuilder.getCardsHtml({
                    items: result.Items,
    ...
    function renderGenres(elem, item, limit, isStatic) {
        for (var context = inferContext(item), html = "", genres = item.Genres || [], i = 0, length = genres.length; i < length && !(limit && i >= limit); i++) {
            i > 0 && (html += '<span class="bulletSeparator">&bull;</span>');
            var param = "Audio" == item.Type || "MusicArtist" == item.Type || "MusicAlbum" == item.Type ? "musicgenre" : "genre";
            if ("Game" == item.MediaType && (param = "gamegenre"), isStatic) html += genres[i];
            else {
                var type;
                switch (context) {
                case "tvshows":
    ...
    function renderAwardSummary(elem, item) {
        item.AwardSummary ? (elem.classList.remove("hide"), elem.innerHTML = globalize.translate("ValueAwards", item.AwardSummary)) : elem.classList.add("hide")
    }
    ...
    function renderCollectionItems(page, parentItem, types, items) {
        ...
        for (i = 0, length = types.length; i < length; i++) {
            var type = types[i],
                typeItems = items.filter(function (curr) {
    ...
    function renderCollectionItemType(page, parentItem, type, items) {
        var html = "";
        html += '<div class="detailSection">', html += '<div style="display:flex;align-items:center;">', html += "<h1>", html += "<span>" + type.name + "</span>", html += "</h1>", html += '<button class="btnAddToCollection autoSize" type="button" is="paper-icon-button-light" style="margin-left:1em;"><i class="md-icon" icon="add">add</i></button>', html += "</div>", html += '<div is="emby-itemscontainer" class="detailSectionContent itemsContainer vertical-wrap">';
    ...
    function renderCriticReviews(page, item, limit) {
        ...
        limit && (options.limit = limit), ApiClient.getCriticReviews(item.Id, options).then(function (result) {
            result.TotalRecordCount || item.CriticRatingSummary || item.AwardSummary ? (page.querySelector("#criticReviewsCollapsible").classList.remove("hide"), renderCriticReviewsContent(page, result, limit)) : page.querySelector("#criticReviewsCollapsible").classList.add("hide")
        })
    }
    ...
    function renderCriticReviewsContent(page, result, limit) {
        for (var html = "", reviews = result.Items, i = 0, length = reviews.length; i < length; i++) {
            ...
            html += "</div>", review.Url && (html += '<div class="secondary listItemBodyText"><a class="textlink" href="' + review.Url + '" target="_blank">' + globalize.translate("ButtonFullReview") + "</a></div>"), html += "</div>", html += "</div>", html += "</div>"
        }
    ...
    function splitVersions(page, params) {
        require(["confirm"], function (confirm) {
            confirm("Are you sure you wish to split the media sources into separate items?", "Split Media Apart").then(function () {
                Dashboard.showLoadingMsg(), ApiClient.ajax({
                    type: "DELETE",
                    url: ApiClient.getUrl("Videos/" + params.id + "/AlternateSources")
                }).then(function () {
                    Dashboard.hideLoadingMsg(), reload(page, params)
                })
            })
        })
    }
        ...
        function (view, params) {
            ...
            function onMoreCommandsClick() {
                ...
                itemContextMenu.show(getContextMenuOptions(currentItem, button)).then(function (result) {
                    result.deleted ? Emby.Page.goHome() : result.updated && reload(view, params)
                })
            }
