define(["libraryBrowser", "libraryMenu", "playbackManager", "emby-tabs", "emby-button"], function (libraryBrowser, libraryMenu, playbackManager) {
    ...
    function loadHomeTab(page, tabContent) {
        if (window.ApiClient) {
            ...
            var promises = [getDisplayPreferences("home", userId), Dashboard.getCurrentUser(), getRequirePromise(["userSettings"])];
            Promise.all(promises).then(function (responses) {
                var displayPreferences = responses[0],
                    user = responses[1],
                    userSettings = responses[2];
                loadSections(tabContent, user, userSettings).then(function () {
                    AppInfo.isNativeApp || showWelcomeIfNeeded(page, displayPreferences), Dashboard.hideLoadingMsg()
                })
            })
        }
    }
    ...
    return function (view, params) {
        ...
        function onPlaybackStop(e, state) {
            state.NowPlayingItem && "Video" == state.NowPlayingItem.MediaType && viewTabs.triggerTabChange()
        }
        ...
        libraryBrowser.configurePaperLibraryTabs(view, viewTabs, view.querySelectorAll(".pageTabContent"), [0, 1, 2, 3], !0);
        var tabControllers = [],
            renderedTabs = [];
        viewTabs.addEventListener("beforetabchange", function (e) {
            ...
            Events.on(playbackManager, "playbackstop", onPlaybackStop), Events.on(ApiClient, "websocketmessage", onWebSocketMessage)
            ...
            headroom.add(viewTabs), self.headroom = headroom
        }), view.addEventListener("viewdestroy", function (e) {
            self.headroom && self.headroom.remove(viewTabs)
        })
    }
});