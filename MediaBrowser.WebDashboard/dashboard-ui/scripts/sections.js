define(["libraryBrowser", "cardBuilder", "appSettings", "components/groupedcards", "dom", "apphost", "imageLoader", "scrollStyles", "emby-button", "paper-icon-button-light", "emby-itemscontainer"], function (libraryBrowser, cardBuilder, appSettings, groupedcards, dom, appHost, imageLoader) {
    ...
    function getLibraryButtonsHtml(items) {
        for (var html = "", i = 0, length = items.length; i < length; i++) {
            ...
            item.CollectionType && (cssClass += " " + item.CollectionType + "buttonCard");
            var href = item.url || libraryBrowser.getHref(item),
                onclick = item.onclick ? ' onclick="' + item.onclick + '"' : "";
            icon = item.icon || icon, html += "<a" + onclick + ' data-id="' + item.Id + '" class="' + cssClass + '" href="' + href + '" style="min-width:12.5%;">', html += '<div class="cardBox ' + cardBuilder.getDefaultColorClass(item.Name) + '" style="margin:4px;">', html += "<div class='cardText'>", html += '<i class="md-icon">' + icon + "</i>", html += '<span style="margin-left:.7em;">' + item.Name + "</span>", html += "</div>", html += "</div>", html += "</a>"
        }
        return html
    }
    ...
    function loadlibraryButtons(elem, userId, index) {
        return getUserViews(userId).then(function (items) {
            var html = "<br/>";
            return index && (html += '<h1 class="listHeader">' + Globalize.translate("HeaderMyMedia") + "</h1>"), html += '<div style="display:flex;flex-wrap:wrap;">', html += getLibraryButtonsHtml(items), html += "</div>", getAppInfo().then(function (infoHtml) {
    ...
    function getAppInfo() {
        var frequency = 864e5;
        AppInfo.isNativeApp && (frequency = 1728e5);
        var cacheKey = "lastappinfopresent5",
            lastDatePresented = parseInt(appSettings.get(cacheKey) || "0");
        return lastDatePresented ? (new Date).getTime() - lastDatePresented < frequency ? Promise.resolve("") : Dashboard.getPluginSecurityInfo().then(function (pluginSecurityInfo) {
            if (appSettings.set(cacheKey, (new Date).getTime()), pluginSecurityInfo.IsMBSupporter) return "";
            var infos = [getPremiereInfo];
            return browserInfo.safari && AppInfo.isNativeApp || infos.push(getTheaterInfo), infos[getRandomInt(0, infos.length - 1)]()
    ...
    function getCard(img, target, shape) {
        ...
        return html += target ? '<a class="cardContent" href="' + target + '" target="_blank">' : '<div class="cardContent">', html += '<div class="cardImage lazy" data-src="' + img + '"></div>', html += target ? "</a>" : "</div>", html += "</div></div></div>"
    }

    function getTheaterInfo() {
        var html = "";
        html += "<div>", html += '<h1>Discover Emby Theater<button is="paper-icon-button-light" style="margin-left:1em;" onclick="this.parentNode.parentNode.remove();" class="autoSize"><i class="md-icon">close</i></button></h1>';
        var nameText = AppInfo.isNativeApp ? "Emby Theater" : '<a href="https://emby.media/download" target="_blank">Emby Theater</a>';
        return html += "<p>A beautiful app for your TV and large screen tablet. " + nameText + " runs on Windows, Xbox One, Raspberry Pi, Samsung Smart TVs, Sony PS4, Web Browsers, and more.</p>", html += '<div class="itemsContainer vertical-wrap">', html += getCard("https://raw.githubusercontent.com/MediaBrowser/Emby.Resources/master/apps/theater1.png", "https://emby.media/download"), html += getCard("https://raw.githubusercontent.com/MediaBrowser/Emby.Resources/master/apps/theater2.png", "https://emby.media/download"), html += getCard("https://raw.githubusercontent.com/MediaBrowser/Emby.Resources/master/apps/theater3.png", "https://emby.media/download"), html += "</div>", html += "<br/>", html += "</div>"
    }

    function getPremiereInfo() {
        var html = "";
        html += "<div>", html += '<h1>Discover Emby Premiere<button is="paper-icon-button-light" style="margin-left:1em;" onclick="this.parentNode.parentNode.remove();" class="autoSize"><i class="md-icon">close</i></button></h1>';
        var cardTarget = AppInfo.isNativeApp ? "" : "https://emby.media/premiere",
            learnMoreText = AppInfo.isNativeApp ? "" : '<a href="https://emby.media/premiere" target="_blank">Learn more</a>';
        return html += "<p>Design beautiful Cover Art, enjoy free access to Emby apps, and more. " + learnMoreText + "</p>", html += '<div class="itemsContainer vertical-wrap">', html += getCard("https://raw.githubusercontent.com/MediaBrowser/Emby.Resources/master/apps/theater1.png", cardTarget), html += getCard("https://raw.githubusercontent.com/MediaBrowser/Emby.Resources/master/apps/theater2.png", cardTarget), html += getCard("https://raw.githubusercontent.com/MediaBrowser/Emby.Resources/master/apps/theater3.png", cardTarget), html += "</div>", html += "<br/>", html += "</div>"
    }
        ...
        return ApiClient.getJSON(ApiClient.getUrl("Users/" + user.Id + "/Items/Latest", options)).then(function (items) {
            ...
            if (items.length) {
                ...
                var viewType = parent.CollectionType,
                    ...
                    cardLayout = supportsImageAnalysis && ("music" === viewType || "movies" === viewType || "tvshows" === viewType || "musicvideos" === viewType || !viewType);
                html += cardBuilder.getCardsHtml({
                    ...
                    showTitle: "music" === viewType || !viewType || cardLayout,
                    showYear: cardLayout && "movies" === viewType,
                    showSeriesYear: cardLayout && "tvshows" === viewType,
                    showParentTitle: "music" === viewType || !viewType || cardLayout && "tvshows" === viewType,
    ...
    function loadRecentlyAdded(elem, user) {
        return elem.classList.remove("homePageSection"), getUserViews(user.Id).then(function (items) {
            for (var excludeViewTypes = ["playlists", "livetv", "boxsets", "channels"], excludeItemTypes = ["Channel"], i = 0, length = items.length; i < length; i++) {
                ...
                if (user.Configuration.LatestItemsExcludes.indexOf(item.Id) === -1 && excludeViewTypes.indexOf(item.CollectionType || []) === -1 && excludeItemTypes.indexOf(item.Type) === -1) {
                    ...
                    frag.classList.add("homePageSection"), elem.appendChild(frag), renderLatestSection(frag, user, item)
                }
            }
        })
    }
    ...
    function loadLatestChannelMedia(elem, userId) {
        ...
        return ApiClient.getJSON(ApiClient.getUrl("Channels/Items/Latest", options)).then(function (result) {
            var html = "";
            result.Items.length && (html += '<h1 class="listHeader">' + Globalize.translate("HeaderLatestChannelMedia") + "</h1>", html += '<div is="emby-itemscontainer" class="itemsContainer vertical-wrap">', html += cardBuilder.getCardsHtml({
                items: result.Items,
                shape: "auto",
                showTitle: !0,
                centerText: !0,
                ...
                showDetailsMenu: !0,
                overlayPlayButton: !0
            }), html += "</div>"), elem.innerHTML = html, imageLoader.lazyChildren(elem)
        })
    }
    ...
    function loadLibraryTiles(elem, user, shape) {
        return getUserViews(user.Id).then(function (items) {
            ...
            if (html += "<div>", items.length) {
                html += "<div>", html += '<h1 class="listHeader">' + Globalize.translate("HeaderMyMedia") + "</h1>", html += "</div>";
                var scrollX = enableScrollX();
                html += scrollX ? '<div is="emby-itemscontainer" class="hiddenScrollX itemsContainer">' : '<div is="emby-itemscontainer" class="itemsContainer vertical-wrap">', html += cardBuilder.getCardsHtml({
                    items: items,
            ...
            return html += "</div>", getAppInfo().then(function (infoHtml) {
                elem.innerHTML = html + infoHtml, imageLoader.lazyChildren(elem)
            })
        })
    }
    ...
    function loadResume(elem, userId) {
        ...
        return ApiClient.getItems(userId, options).then(function (result) {
            ...
            if (result.Items.length) {
                html += '<h1 class="listHeader">' + Globalize.translate("HeaderContinueWatching") + "</h1>", html += enableScrollX() ? '<div is="emby-itemscontainer" class="hiddenScrollX itemsContainer">' : '<div is="emby-itemscontainer" class="itemsContainer vertical-wrap">';
    ...
    function loadLatestChannelItemsFromChannel(page, channel, index) {
        var screenWidth = dom.getWindowSize().innerWidth,
            options = {
                Limit: screenWidth >= 1600 ? 10 : screenWidth >= 1440 ? 5 : 6,
                Fields: "PrimaryImageAspectRatio,BasicSyncInfo",
                Filters: "IsUnplayed",
        ...
        ApiClient.getJSON(ApiClient.getUrl("Channels/Items/Latest", options)).then(function (result) {
            var html = "";
            if (result.Items.length) {
                html += '<div class="homePageSection">', html += "<div>";
                var text = Globalize.translate("HeaderLatestFromChannel").replace("{0}", channel.Name);
                html += '<h1 style="display:inline-block; vertical-align:middle;" class="listHeader">' + text + "</h1>", html += '<a href="channelitems.html?id=' + channel.Id + '" class="clearLink" style="margin-left:2em;"><button is="emby-button" type="button" class="raised more mini"><span>' + Globalize.translate("ButtonMore") + "</span></button></a>", html += "</div>", html += '<div is="emby-itemscontainer" is="emby-itemscontainer" class="itemsContainer vertical-wrap">', html += cardBuilder.getCardsHtml({
                    items: result.Items,
                    shape: "autohome",
                    defaultShape: "square",
                    showTitle: !0,
