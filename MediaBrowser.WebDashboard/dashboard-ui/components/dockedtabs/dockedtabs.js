define(["apphost", "connectionManager", "events", "globalize", "browser", "require", "dom", "embyRouter", "emby-tabs"], function (appHost, connectionManager, events, globalize, browser, require, dom, embyRouter) {
    ...
    function showMoreMenu(button) {
        var commands = [];
        currentUser.Policy.IsAdministrator && (commands.push({
            name: globalize.translate("ButtonManageServer"),
            id: "manageserver"
        }), dom.getWindowSize().innerWidth >= 1e3 && commands.push({
            name: globalize.translate("MetadataManager"),
            id: "metadatamanager"
        }), commands.push({
            name: globalize.translate("ButtonReports"),
            id: "reports"
        })), appHost.supports("multiserver") && commands.push({
            name: globalize.translate("HeaderSelectServer"),
            id: "selectserver"
        }), commands.push({
            name: globalize.translate("TabSettings"),
            id: "settings"
        }), currentUser.Policy.EnableSync && commands.push({
    ...
    function onTabClick(e) {
        ...
        case 0:
            ...
            break;
        case 1:
            showLibrariesMenu(this), e.preventDefault(), e.stopPropagation();
            break;
        case 2:
        ...
        case 4:
            ...
            break;
        case 5:
            showMoreMenu(this), e.preventDefault(), e.stopPropagation()
        }
    }
    ...
    function addNoFlexClass(buttons) {
        setTimeout(function () {
            for (var i = 0, length = buttons.length; i < length; i++) {
                ...
                button.classList.contains("emby-button-noflex") && button.classList.add("dockedtabs-tab-button-noflex")
            }
        }, 300)
    }
    ...
    function showUserTabs(user, element) {
        ...
        apiClient.getUserViews({}, user.Id).then(function (result) {
            currentUserViews = result.Items, onUserViewResponse(user, result.Items, element)
        }, function () {
            currentUserViews = [], onUserViewResponse(user, [], element)
        })
    }

    function showCurrentUserTabs(element) {
        Dashboard.getCurrentUserId() && Dashboard.getCurrentUser().then(function (user) {
            showUserTabs(user, element)
        })
    }

    function onViewShow(e) {
        e.detail.properties.indexOf("fullscreen") === -1 && Dashboard.getCurrentUserId() ? instance.show() : instance.hide()
    }

    function dockedTabs(options) {
        var self = this;
        instance = self, self.element = render(options), events.on(connectionManager, "localusersignedin", function (e, user) {
            self.show(), showUserTabs(user, self.element)
        }), events.on(connectionManager, "localusersignedout", function () {
            self.hide()
        }), showCurrentUserTabs(self.element), document.addEventListener("viewshow", onViewShow)
    }
    require(["css!./dockedtabs"]);
    var instance, currentUser = {},
        currentUserViews = [];
    return dockedTabs.prototype.destroy = function () {
        document.removeEventListener("viewshow", onViewShow), instance = null;
        var self = this;
        self.element;
        self.element = null
