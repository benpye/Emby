define(["browser", "datetime", "backdrop", "libraryBrowser", "listView", "userdataButtons", "imageLoader", "playbackManager", "nowPlayingHelper", "events", "connectionManager", "apphost", "globalize", "cardStyle", "emby-itemscontainer", "css!css/nowplaying.css"], function (browser, datetime, backdrop, libraryBrowser, listView, userdataButtons, imageLoader, playbackManager, nowPlayingHelper, events, connectionManager, appHost, globalize) {
    ...
    function showAudioMenu(context, player, button, item) {
        var currentIndex = playbackManager.getAudioStreamIndex(player),
            streams = (item.MediaStreams || []).filter(function (i) {
                return "Audio" == i.Type
            }),
    ...
    function updateNowPlayingInfo(context, state) {
        ...
        url !== currentImgUrl && (setImageUrl(context, url), item ? (backdrop.setBackdrops([item]), ApiClient.getItem(Dashboard.getCurrentUserId(), item.Id).then(function (fullItem) {
        ...
        })) : (backdrop.clear(), userdataButtons.destroy({
            element: context.querySelector(".nowPlayingPageUserDataButtons")
        })))
    }
    ...
    function updateSupportedCommands(context, commands) {
        for (var all = context.querySelectorAll(".btnCommand"), i = 0, length = all.length; i < length; i++) buttonEnabled(all[i], commands.indexOf(all[i].getAttribute("data-command")) != -1)
    }
    var currentImgUrl;
    return function () {
        ...
        function updatePlayerState(context, state) {
            ...
            buttonVisible(context.querySelector(".btnToggleFullscreen"), item && "Video" == item.MediaType && supportedCommands.indexOf("ToggleFullscreen") != -1), buttonVisible(context.querySelector(".btnAudioTracks"), hasStreams(item, "Audio") && supportedCommands.indexOf("SetAudioStreamIndex") != -1), buttonVisible(context.querySelector(".btnSubtitles"), hasStreams(item, "Subtitle") && supportedCommands.indexOf("SetSubtitleStreamIndex") != -1), supportedCommands.indexOf("DisplayMessage") != -1 ? context.querySelector(".sendMessageSection").classList.remove("hide") : context.querySelector(".sendMessageSection").classList.add("hide"), supportedCommands.indexOf("SendString") != -1 ? context.querySelector(".sendTextSection").classList.remove("hide") : context.querySelector(".sendTextSection").classList.add("hide"), buttonVisible(context.querySelector(".btnStop"), null != item), buttonVisible(context.querySelector(".btnNextTrack"), null != item), buttonVisible(context.querySelector(".btnPreviousTrack"), null != item);
            var positionSlider = context.querySelector(".nowPlayingPositionSlider");
            positionSlider && !positionSlider.dragging && (positionSlider.disabled = !playState.CanSeek), updatePlayPauseState(playState.IsPaused, null != item);
            var runtimeTicks = item ? item.RunTimeTicks : null;
            updateTimeDisplay(playState.PositionTicks, runtimeTicks), updatePlayerVolumeState(context, playState.IsMuted, playState.VolumeLevel), item && "Video" == item.MediaType ? context.classList.remove("hideVideoButtons") : context.classList.add("hideVideoButtons"), updateRepeatModeDisplay(playState.RepeatMode), updateNowPlayingInfo(context, state)
        }

        function updateRepeatModeDisplay(repeatMode) {
            var context = dlg,
                toggleRepeatButton = context.querySelector(".repeatToggleButton");
            "RepeatAll" == repeatMode ? (toggleRepeatButton.innerHTML = "<i class='md-icon'>repeat</i>", toggleRepeatButton.classList.add("nowPlayingPageRepeatActive")) : "RepeatOne" == repeatMode ? (toggleRepeatButton.innerHTML = "<i class='md-icon'>repeat_one</i>", toggleRepeatButton.classList.add("nowPlayingPageRepeatActive")) : (toggleRepeatButton.innerHTML = "<i class='md-icon'>repeat</i>", toggleRepeatButton.classList.remove("nowPlayingPageRepeatActive"))
        }
        ...
        function updatePlayPauseState(isPaused, isActive) {
            ...
            isPaused ? btnPlayPause.querySelector("i").innerHTML = "play_arrow" : btnPlayPause.querySelector("i").innerHTML = "pause", buttonVisible(btnPlayPause, isActive)
        }
        ...
        function updateTimeDisplay(positionTicks, runtimeTicks) {
            ...
            null == positionTicks ? context.querySelector(".positionTime").innerHTML = "--:--" : context.querySelector(".positionTime").innerHTML = datetime.getDisplayRunningTime(positionTicks), null != runtimeTicks ? context.querySelector(".runtime").innerHTML = datetime.getDisplayRunningTime(runtimeTicks) : context.querySelector(".runtime").innerHTML = "--:--"
        }
        ...
        function onPlaybackStopped(e, state) {
            console.log("remotecontrol event: " + e.type);
            state.NextMediaType || (updatePlayerState(dlg, {}), loadPlaylist(dlg))
        }

        function onPlayPauseStateChanged(e) {
            var player = this;
            updatePlayPauseState(player.paused(), !0)
        }

        function onStateChanged(event, state) {
            var player = this;
            updatePlayerState(dlg, state), loadPlaylist(dlg, player)
        }
        ...
        function bindToPlayer(context, player) {
            if (releaseCurrentPlayer(), currentPlayer = player, player) {
                ...
                }), events.on(player, "playbackstart", onPlaybackStart), events.on(player, "statechange", onStateChanged), events.on(player, "repeatmodechange", onRepeatModeChange), events.on(player, "playlistitemremove", onPlaylistItemRemoved), events.on(player, "playlistitemmove", onPlaylistUpdate), events.on(player, "playbackstop", onPlaybackStopped), events.on(player, "volumechange", onVolumeChanged), events.on(player, "pause", onPlayPauseStateChanged), events.on(player, "playing", onPlayPauseStateChanged), events.on(player, "timeupdate", onTimeUpdate);
        ...
        function updateCastIcon(context) {
            ...
            info && !info.isLocalPlayer ? (btnCast.querySelector("i").innerHTML = "cast_connected", btnCast.classList.add("btnActiveCast")) : (btnCast.querySelector("i").innerHTML = "cast", btnCast.classList.remove("btnActiveCast"))
        }
        ...
        function onPlayerChange() {
            var context = dlg;
            updateCastIcon(context), bindToPlayer(context, playbackManager.getCurrentPlayer())
        }
        ...
        function init(ownerView, context) {
            bindEvents(context), context.querySelector(".sendMessageForm").addEventListener("submit", onMessageSubmit), context.querySelector(".typeTextForm").addEventListener("submit", onSendStringSubmit), context.querySelector(".btnCast").addEventListener("click", function () {
                var btn = this;
                ...
                history.back()
            }), events.on(playbackManager, "playerchange", onPlayerChange), appHost.supports("remotecontrol") && context.querySelector(".btnCast").classList.remove("hide")
        }
        ...
        function onShow(context, tab) {
            currentImgUrl = null, bindToPlayer(context, playbackManager.getCurrentPlayer()), updateCastIcon(context)
        }
